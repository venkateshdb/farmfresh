...
def seller():
    ...
    if request.method == "POST":
        ...
        if not seller_img:
            seller = Seller(full_name, city, state, address,
                            phone, password, seller_img_name="",is_verified=0)
        else:
            seller = Seller(full_name, city, state, address, phone,
                            password, seller_img_name=seller_img.filename, is_verified=0)
...
def buyer():
    ...
    if buyer_img == "":
        buyer = Buyer(full_name, city, state, address,
                      phone, password, buyer_img_name="")
    else:
        buyer = Buyer(full_name, city, state, address, phone,
                      password,  buyer_img_name=buyer.filename)
...
def send_otp(otp):
    message = client.messages.create(body="Your OTP For FarmFresh is {0}".format(
        otp), from_="+12132386072", to="+918208264232")
    print(message)
    return message.sid
    ...
    if request.method == "POST":
        ...
        if get:
            ...
            if session.get("type") == "seller":
            	Seller.query.filter_by(phone_num=sess_phone).update({"is_verified":1})
            elif(session.get("type") == "buyer"):
            	Buyer.query.filter_by(phone_num=sess_phone).update({"is_verified":1})
